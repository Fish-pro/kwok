...
function test_restart() {
  ...
  echo  kwokctl stop cluster --name "${name}"
  kwokctl stop cluster --name "${name}"
  if [ $? -eq 0 ]; then
  ...
  echo kwokctl start cluster --name "${name}" --wait 10m
  kwokctl start cluster --name "${name}" --wait 10m
  if [ $? -eq 0 ]; then
...
function test_restart_components() {
    local name="${1}"
    components=(
        "etcd"
        "kube-apiserver"
    ...
    kwokctl stop cluster --name "${name}" "${args[@]}" || true
    sleep 30
    test_prometheus
    if [ $? -eq 0 ]; then
        echo "Error: cluster ${name} components stop error"
        return 1
    else
        echo "Cluster ${name} components stopped successfully."
    fi

    kwokctl start cluster --name "${name}" "${args[@]}" || true
    sleep 30
    test_prometheus
    if [ $? -eq 0 ]; then
        echo "Cluster ${name} components started successfully."
    else
        echo "Error: cluster ${name} components start error"
        return 1
    fi
}
...
function main() {
  ...
  for release in "${RELEASES[@]}"; do
    ...
    test_restart "${release}" "${name}" || failed+=("restart_${name}")
    test_restart_components "${name}" || failed+=("restart_components_${name}")
