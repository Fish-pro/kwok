...
func NewCommand(ctx context.Context) *cobra.Command {
	...
	cmd := &cobra.Command{
		Args:  cobra.NoArgs,
		Use:   "cluster",
		Short: "Start a cluster or cluster component(s)",
		Long:  "Start a cluster or cluster component(s)",
...
func runE(ctx context.Context, flags *flagpole) error {
	...
	if len(flags.Components) != 0 {
		...
		err = rt.StartComponents(ctx, flags.Components...)
		if err != nil {
			logger.Error("Starting cluster component(s)", err, "components", flags.Components)
		} else {
			logger.Info("Cluster component(s) Started", "components", flags.Components)
		}
		return nil
	}
